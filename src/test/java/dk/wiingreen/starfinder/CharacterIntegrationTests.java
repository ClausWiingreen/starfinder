...
class CharacterIntegrationTests {
  ...
  public CharacterIntegrationTests(
      UserRepository userRepository, MockMvc mockMvc, CharacterRepository characterRepository) {
  ...
  void characterIsSavedForLoggedInUserWhenFormIsSubmitted() throws Exception {
    ...
    mockMvc
        .perform(post("/characters/new").param("name", "Nova Vance").with(csrf()))
  ...
  void characterNameIsUpdatedWhenEditFormIsSubmitted() throws Exception {
    ...
    var character = characterRepository.save(user.createCharacter("Old Name"));

    mockMvc
        .perform(post("/characters/{id}", character.getId()).param("name", "New Name").with(csrf()))
        .andExpect(status().is3xxRedirection());

    var updated = characterRepository.findById(character.getId());

    assertThat(updated)
        .hasValueSatisfying(ch -> assertThat(ch).extracting("name").isEqualTo("New Name"));
  }

  @Test
  void unauthenticatedUserCannotSubmitCharacterForm() throws Exception {
    mockMvc
        .perform(post("/characters").param("name", "Hacker Mouse").with(csrf()))
        .andExpect(status().is3xxRedirection());
  }
  ...
  void userCannotEditAnotherUsersCharacter() throws Exception {
    ...
    var character = characterRepository.save(owner.createCharacter("Original Name"));

    mockMvc
        .perform(
            post("/characters/{id}", character.getId()).param("name", "Hacked Name").with(csrf()))
        .andExpect(status().is2xxSuccessful());

    assertThat(characterRepository.findById(character.getId()))
        .hasValueSatisfying(
            storedCharacter -> {
              assertThat(storedCharacter).extracting("name").isEqualTo("Original Name");
            });
  }
  ...
  void characterFormRejectsEmptyName() throws Exception {
    ...
    mockMvc
        .perform(post("/characters/new").param("name", "").with(csrf()))
  ...
  void editFormRejectsEmptyName() throws Exception {
    var user = setupUser("testuser");
    var character = characterRepository.save(user.createCharacter("Valid Name"));

    mockMvc
        .perform(post("/characters/{id}", character.getId()).param("name", "").with(csrf()))
        .andExpect(status().isOk())
    ...
    assertThat(characterRepository.findById(character.getId()))
        .hasValueSatisfying(
            storedCharacter ->
                assertThat(storedCharacter).extracting("name").isEqualTo("Valid Name"));
  }
  ...
  void userCanDeleteOwnCharacter() throws Exception {
    ...
    mockMvc
        .perform(post("/characters/{id}/delete", character.getId()).with(csrf()))
        .andExpect(status().is3xxRedirection())
  ...
  void userCannotDeleteOthersCharacter() throws Exception {
    ...
    assertThat(characterRepository.findById(character.getId())).hasValue(character);
  }
  ...
  void deleteRedirectsEvenIfCharacterDoesNotExist() throws Exception {
    ...
    mockMvc
        .perform(post("/characters/{id}/delete", nonExistentId).with(csrf()))
        .andExpect(status().is3xxRedirection())
        .andExpect(redirectedUrl("/characters"));
  }
